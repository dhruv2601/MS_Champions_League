...
class PRN:
    ...
    def process(self, input, image_info=None):
        ...
        print('process size: ' + str(image.shape))
        print('proecss dim: ' + str(image.ndim))

        if image.ndim < 3:
            print('process size 2: ' + str(image.shape))
            image = np.tile(image[:, :, np.newaxis], [1, 1, 3])
            print('process size 3: ' + str(image.shape))
        ...
        image = image / 255.
        print('pre crop size:' + str(image.shape))
